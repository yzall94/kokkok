...
export async function GET() {
  ...
  try {
    const [totalRes, matchedRes] = await Promise.all([
      db.execute('SELECT COUNT(*) as cnt FROM kokkok_entries'),
      db.execute('SELECT COUNT(*) as cnt FROM kokkok_entries WHERE matched = 1'),
    ])

    const total = (totalRes.rows[0].cnt as number) + CAL_A
    const couples = Math.floor((matchedRes.rows[0].cnt as number) / 2) + CAL_B
